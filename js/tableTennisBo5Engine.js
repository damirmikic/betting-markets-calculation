/**
 * Table tennis BO5 pricing engine based on odds -> probabilities -> distributions.
 * All odds are decimal. All probabilities are fair unless a margin is explicitly applied.
 */
...
/**
 * Apply overround to a binary market (two outcomes).
 * targetOverround = e.g. 1.05 for 5% margin.
 */
function applyMarginBinary(p1, p2, targetOverround = 1.05) {
    const factor = targetOverround; // since p1 + p2 = 1
    const p1m = p1 * factor;
    const p2m = p2 * factor;
    return {
        p1: p1m,
...
/**
 * Apply overround to a multi-outcome market.
 * probs: array of fair probabilities that sum to 1
 * targetOverround: e.g. 1.07
 */
function applyMarginMulti(probs, targetOverround = 1.07) {
    const factor = targetOverround; // since sum(probs) = 1
    return probs.map((p) => {
        const pm = p * factor;
        return {
            p: pm,
...
/**
 * Given match win probability (home) pMatch,
 * invert the BO5 polynomial to get set win probability s.
 *
 * Match polynomial:
 *   P_match(s) = 6 s^5 - 15 s^4 + 10 s^3
 */
function invertBo5ForSetProb(pMatch, tol = 1e-8, maxIter = 100) {
    ...
    // We know favorite must have s >= 0.5
    let lo = 0.5;
    let hi = 1 - 1e-9;
    ...
    let fHi = f(hi);
    if (fLo * fHi > 0) {
        // Should never happen for reasonable pMatch, but guard anyway
...
/**
 * Given set win prob s for home, return full BO5 distribution.
 */
...
function priceSetsHandicap(s, correctScore, marginBinary = 1.05) {
    const pHomeMinus1_5 =
        correctScore['3-0'] + correctScore['3-1'];
    const pHomeMinus2_5 = correctScore['3-0'];

    const m1_5 = applyMarginBinary(pHomeMinus1_5, 1 - pHomeMinus1_5, marginBinary);
    const m2_5 = applyMarginBinary(pHomeMinus2_5, 1 - pHomeMinus2_5, marginBinary);
...
function pricePointsMarkets(s, totalSetsDist, pointsParams = { a: 15.5, b: 7.5 }) {
